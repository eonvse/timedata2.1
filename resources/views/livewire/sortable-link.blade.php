<div class="flex text-cyan-900 dark:text-cyan-200 text-xs">
    <span class="underline {{ isset($sortDirection) && $field==$sortField ? 'decoration-solid' : 'decoration-dotted' }} hover:cursor-pointer" title="Сортировать" wire:click="sort">{{ $text }}</span>
    @if(isset($sortDirection) && $field==$sortField)
